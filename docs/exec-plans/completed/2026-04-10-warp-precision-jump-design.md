...
### Phase 2 — Jump (when user clicks jump in Aisland)
...
`CGEventPost` requires macOS Accessibility permission for the parent process (Aisland). On first jump attempt, if `AXIsProcessTrustedWithOptions` returns false:

- Show a one-time in-app prompt explaining why the permission is needed ("so Aisland can focus the exact Warp tab for your agent session")
...
**`Sources/AislandCore/WarpSQLiteReader.swift`** (~150 lines)
...
**`Sources/AislandApp/AccessibilityPermissionCoordinator.swift`** (~80 lines)
...
**`Sources/AislandCore/JumpTarget.swift`** — add one field:
```swift
public var warpPaneUUID: String?
```
Stable field; backward-compatible decoding (nil default).

**`Sources/AislandCore/ClaudeHooks.swift`** — in `withRuntimeContext`, after the existing `terminalApp` inference:
```swift
...
**`Sources/AislandCore/CodexHooks.swift`** — same addition, so Codex sessions in Warp get the same precision jump. (Open Code can follow in a separate slice if we want to limit scope.)

**`Sources/AislandApp/TerminalJumpService.swift`** — in the `switch descriptor.bundleIdentifier` at line ~200, add:
```swift
...
**`Sources/AislandApp/KeystrokeInjector.swift`** (new, ~40 lines) — thin wrapper around `CGEventCreateKeyboardEvent` / `CGEventPost` with a single public method `sendCmdShiftRightBracket()`. Isolated so it's easy to mock in tests.

### Test changes

- `Tests/AislandCoreTests/WarpSQLiteReaderTests.swift` (new) — creates a temp SQLite database seeded with synthetic `windows`/`tabs`/`pane_nodes`/`terminal_panes`/`commands`/`blocks` rows reproducing the real schema, then asserts `lookupPaneUUID` and `currentFocusedPaneUUID` return the expected values for various scenarios (single window single tab, multiple tabs, non-Warp cwd, nonexistent file, etc.)
- `Tests/AislandAppTests/TerminalJumpServiceTests.swift` — add tests for the Warp branch using an injected `WarpSQLiteReader` protocol stub and a `KeystrokeInjector` spy. Cover: already-at-target (no keystrokes), reaches-target-after-2-cycles, never-reaches-target (falls through cleanly).
...
2. Launch the development `AislandApp` build.
3. Grant Accessibility permission when prompted.
4. Click jump on the 2nd session in the Aisland session list while the 4th session is currently focused in Warp.
...
1. **Upstream feature request to Warp** — draft and file a GitHub issue requesting a `warp://action/focus_cli_agent?session_id=…` URL scheme action. Point out that `CLIAgentSessionsModel` already tracks every session internally, and a public entry point would allow any external integration (not just Aisland) to do one-click precision jump without scraping SQLite. Reference the `claude-code-warp` plugin's current one-way integration as the motivating use case.
...
4. **Settings for cycle keystroke** — expose a text field in Aisland settings for the cycle keystroke, default `Cmd+Shift+]`.
