...
Reviewed on 2026-04-02 in worktree `../aisland-island-investigate` on branch `investigate/island-position`.

## Question

Why does the current open-source island feel "off" compared with the official Aisland behavior on a MacBook with a built-in notch?
...
Current positioning logic in `Sources/AislandApp/OverlayPanelController.swift`:
