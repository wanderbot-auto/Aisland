...
[`notchi`](https://github.com/sk-ruban/notchi) is a strong adjacent reference for Aisland:
...
Aisland should learn from that execution without collapsing into a Claude-specific mascot app.
...
That is a useful product pattern for Aisland:

1. ambient awareness in the notch
2. actionable detail on expand
3. local transport and setup automation underneath

## Where Aisland Should Differ

Notchi is optimized for Claude Code presence and delight. Aisland should stay optimized for control.

Aisland should keep these distinctions:
...
Aisland already has the beginning of the Codex side:

- [Sources/AislandCore/CodexHooks.swift](/Users/wangruobing/Personal/aisland/Sources/AislandCore/CodexHooks.swift)
- [Sources/AislandHooks/main.swift](/Users/wangruobing/Personal/aisland/Sources/AislandHooks/main.swift)

The next step is to add a native installer that:

- detects Codex config presence
- installs the `AislandHooks` executable or a thin wrapper script
...
Aisland should do a control-oriented version of that:
...
Aisland can adopt that idea in a more neutral form:
...
Notchi demonstrates that a narrow first integration is enough to ship. Aisland should still keep its bridge generic so Claude Code or other agents can be added later as adapters.

The current shared model and bridge transport already point in the right direction:

- [Sources/AislandCore/AgentEvent.swift](/Users/wangruobing/Personal/aisland/Sources/AislandCore/AgentEvent.swift)
- [Sources/AislandCore/BridgeTransport.swift](/Users/wangruobing/Personal/aisland/Sources/AislandCore/BridgeTransport.swift)
- [Sources/AislandCore/SessionState.swift](/Users/wangruobing/Personal/aisland/Sources/AislandCore/SessionState.swift)
...
That would capture the highest-leverage lessons from Notchi while staying aligned with Aisland's stated product direction.
...
Those features may help Notchi's product identity, but they are not on Aisland's current critical path.
