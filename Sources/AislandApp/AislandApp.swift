...
@MainActor
final class AislandAppDelegate: NSObject, NSApplicationDelegate {
    let model = AppModel()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DispatchQueue.main.async { [self] in
            ...
            AislandAppDelegate.hideAllAppWindows()
...
struct AislandApp: App {
    @NSApplicationDelegateAdaptor(AislandAppDelegate.self)
    private var appDelegate

    @Environment(\.openWindow) private var openWindow

    var body: some Scene {
        Window("Aisland Settings", id: "settings") {
        ...
        MenuBarExtra {
            ...
            AislandBrandMark(size: 18, style: .template)
                .accessibilityLabel("Aisland")
        }
