...
struct HarnessLaunchConfiguration {
    ...
    init(environment: [String: String] = ProcessInfo.processInfo.environment) {
        scenario = Self.scenarioValue(from: environment["AISLAND_HARNESS_SCENARIO"])
        presentOverlay = Self.boolValue(
            environment["AISLAND_HARNESS_PRESENT_OVERLAY"],
            default: false
        )
        shouldShowControlCenter = Self.boolValue(
            environment["AISLAND_HARNESS_SHOW_CONTROL_CENTER"],
            default: true
        )
        shouldStartBridge = Self.boolValue(
            environment["AISLAND_HARNESS_START_BRIDGE"],
            default: true
        )
        shouldPerformBootAnimation = Self.boolValue(
            environment["AISLAND_HARNESS_BOOT_ANIMATION"],
            default: true
        )
        captureDelay = Self.timeIntervalValue(
            from: environment["AISLAND_HARNESS_CAPTURE_DELAY_SECONDS"]
        )
        autoExitAfter = Self.timeIntervalValue(
            from: environment["AISLAND_HARNESS_AUTO_EXIT_SECONDS"]
        )
        artifactDirectoryURL = Self.directoryURLValue(
            from: environment["AISLAND_HARNESS_ARTIFACT_DIR"]
        )
    }
