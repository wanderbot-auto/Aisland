...
/// Aisland host app authorizes `click menu item` on other
...
public struct DefaultKeystrokeInjector: KeystrokeInjector {
    ...
    public func sendCmdShiftRightBracket() {
        ...
        guard let script = NSAppleScript(source: source) else {
            NSLog("[Aisland] Warp tab advance: NSAppleScript compilation returned nil")
            return
        }
        script.executeAndReturnError(&error)
        if let error {
            NSLog("[Aisland] Warp tab advance failed: %@", String(describing: error))
        }
    }
}
