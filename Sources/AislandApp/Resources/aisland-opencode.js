// Aisland plugin for OpenCode
// Bridges OpenCode events to the Aisland desktop app via Unix socket.
// Install: copy to ~/.config/opencode/plugins/aisland.js
import { connect } from "net";
import { appendFileSync } from "fs";
import { homedir } from "os";

const DEBUG_LOG = "/tmp/aisland-opencode-debug.log";
function debugLog(msg) {
  try { appendFileSync(DEBUG_LOG, `[${new Date().toISOString()}] ${msg}\n`); } catch {}
}

const SOCKET_PATH =
  process.env.AISLAND_SOCKET_PATH ||
  `${process.env.HOME || homedir()}/Library/Application Support/Aisland/bridge.sock`;
  ...
  return {
    "event": async ({ event }) => {
      try {
        ...
        // Fail open: if Aisland is unavailable, don't block OpenCode
      }
    },

    "shell.env": async (input, output) => {
      output.env.AISLAND_ACTIVE = "1";
