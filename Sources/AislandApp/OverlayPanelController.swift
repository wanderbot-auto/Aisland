...
final class OverlayPanelController {
    ...
    /// Cap the opened session list so the island remains glanceable.
    ...
    private static let openedUsageHeight: CGFloat = 380
    ...
    private func openedContentHeight(for model: AppModel) -> CGFloat {
        ...
        if model.islandSurface == .usage {
            return Self.openedUsageHeight
        }

        if model.islandSurface == .whiteNoise {
            return Self.openedWhiteNoiseHeight
        }
    ...
    private func actionableBodyHeight(for session: AgentSession, model: AppModel) -> CGFloat {
        ...
        case .waitingForAnswer:
            return questionCardHeight(
                for: session.questionPrompt,
                optionLayout: model.questionOptionLayout
            ) - 44
        case .completed:
    ...
    private func questionCardHeight(
        for prompt: QuestionPrompt?,
        optionLayout: QuestionOptionLayout
    ) -> CGFloat {
        QuestionCardLayoutMetrics.estimatedHeight(
            for: prompt,
            optionLayout: optionLayout,
            availableWidth: Self.preferredNotificationPanelWidth - 112
        )
    }
