...
final class HookInstallationCoordinator {
    ...
    var claudeHookStatusSummary: String {
        ...
        if hooksBinaryURL == nil {
            return "Build AislandHooks before installing."
        }
    ...
    func installCodexHooks() {
        guard let hooksBinaryURL else {
            onStatusMessage?("Could not find a local AislandHooks binary. Build the package first.")
            return
        }
    ...
    private func loadBundledOpenCodePlugin() -> Data? {
        ...
        if let url = Bundle.appResources.url(forResource: "aisland-opencode", withExtension: "js") {
            return try? Data(contentsOf: url)
        }

        // Fallback: Bundle.main for Xcode builds
        if let url = Bundle.main.url(forResource: "aisland-opencode", withExtension: "js") {
            return try? Data(contentsOf: url)
        }
