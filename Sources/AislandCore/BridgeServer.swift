...
public final class BridgeServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "app.aisland.bridge.server")
            ...
            broadcast([.event(
                resolution.isApproved
                    ...
                    : .sessionCompleted(
                        SessionCompleted(
                            sessionID: sessionID,
                            summary: "Permission denied in Aisland.",
                            timestamp: .now
                        )
                    )
            )])
    ...
    private func resolvePendingOpenCodeInteraction(
        ...
        case let (.permission(_), .deny(message, _)):
            directive = .deny(reason: message ?? "Permission denied in Aisland.")
            summary = message ?? "Permission denied in Aisland."
            phase = .completed
    ...
    private func resolvePendingApproval(sessionID: String, approved: Bool) {
        ...
        if approved {
            ...
            response = .codexHookDirective(.deny(reason: "Permission denied in Aisland."))
        }
    ...
    private func resolvePendingClaudeInteraction(
        ...
        case let (.permission(_), .deny(message, interrupt)):
            directive = .permissionRequest(
                .deny(message: message ?? "Permission denied in Aisland.", interrupt: interrupt)
            )
            summary = message ?? "Permission denied in Aisland."
            phase = .completed
