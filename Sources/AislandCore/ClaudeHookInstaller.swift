...
public struct ClaudeHookInstallerManifest: Equatable, Codable, Sendable {
    public static let fileName = "aisland-claude-hooks-install.json"
...
public enum ClaudeHookInstaller {
    ...
    private static func isLegacyAislandHookCommand(_ command: String) -> Bool {
        ...
        if (normalized.contains("aislandhooks") || normalized.contains("vibeislandhooks")) && normalized.contains("--source claude") {
            return true
        }
