...
public struct HookHealthReport: Equatable, Sendable {
    ...
    public enum Issue: Equatable, Sendable, CustomStringConvertible {
        ...
        /// Other hooks detected alongside Aisland hooks (informational).
...
public enum HookHealthCheck {
    ...
    public static func checkClaude(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: settingsPath),
           hasAislandHooks(in: settingsURL, fileManager: fileManager) {
    ...
    public static func checkCodex(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: hooksPath),
           hasAislandHooks(in: hooksURL, fileManager: fileManager) {
    ...
    public static func checkOpenCode(
        ...
        let pluginFileURL = pluginsDir.appendingPathComponent("aisland.js")
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Only check Aisland / Vibe Island commands
                    let normalized = command.lowercased()
                    guard normalized.contains("aislandhooks") || normalized.contains("vibeislandhooks")
                        || normalized.contains("aisland") || normalized.contains("vibe-island") else {
                        continue
                    }
    ...
    /// Finds third-party (non-Aisland) hook command names for display.
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Skip Aisland / Vibe Island hooks
                    if normalized.contains("aislandhooks") || normalized.contains("vibeislandhooks")
                        || normalized.contains("aisland") || normalized.contains("vibe-island") {
                        continue
                    }
    ...
    /// Checks whether any Aisland managed hooks exist in a config file.
    private static func hasAislandHooks(in url: URL, fileManager: FileManager) -> Bool {
