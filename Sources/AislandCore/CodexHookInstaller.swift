...
public struct CodexHookInstallerManifest: Equatable, Codable, Sendable {
    public static let fileName = "aisland-install.json"
...
public enum CodexHookInstaller {
    ...
    public static let managedStatusMessage = "Managed by Aisland"
    ...
    private static func isManagedHookForInstall(_ hook: [String: Any], replacingCommand: String) -> Bool {
        ...
        return isLegacyAislandHookCommand(command)
    }

    private static func isLegacyAislandHookCommand(_ command: String) -> Bool {
        let normalized = command.lowercased()
        if normalized.contains("aislandhooks") || normalized.contains("vibeislandhooks") {
            return true
        }

        return normalized.contains("aisland-bridge") || normalized.contains("vibe-island-bridge")
    }
