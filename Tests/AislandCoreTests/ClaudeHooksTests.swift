...
struct ClaudeHooksTests {
    ...
    func claudeHookOutputEncoderEncodesPermissionDecision() throws {
        let output = try ClaudeHookOutputEncoder.standardOutput(
            for: .claudeHookDirective(
                .permissionRequest(
                    .deny(message: "Permission denied in Aisland.", interrupt: true)
                )
            )
        )
        ...
        #expect(decision?["message"] as? String == "Permission denied in Aisland.")
    ...
    func claudeHookInstallationManagerRoundTripsInstallAndUninstall() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("aisland-claude-hooks-\(UUID().uuidString)", isDirectory: true)
    ...
    func claudeTranscriptDiscoveryRecoversRecentSessions() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("aisland-claude-discovery-\(UUID().uuidString)", isDirectory: true)
