...
struct SessionStateTests {
    @Test
    func legacyUnknownAgentToolDecodesAsGeneralAgent() throws {
        let json = """
        {
          "id": "legacy-session",
          ...
          "phase": "completed",
          "summary": "Recovered from legacy persisted data.",
          "updatedAt": 1000
        }
        """
    ...
    func actionableStateResolvedClearsWaitingForApproval() {
        ...
        state.apply(
            .actionableStateResolved(
                ActionableStateResolved(
                    ...
                    summary: "Approval was handled outside Aisland.",
                    timestamp: startedAt.addingTimeInterval(10)
                )
            )
        )
        ...
        #expect(state.session(id: "claude-approval")?.summary == "Approval was handled outside Aisland.")
    }
    ...
    func codexPreToolUseWaitsForApprovalAndReturnsDenyDirective() async throws {
        ...
        #expect(response == .codexHookDirective(.deny(reason: "Permission denied in Aisland.")))
    }
    ...
    func codexHookInstallerMergesManagedGroupsWithoutDroppingUnrelatedHooks() throws {
        ...
        let managedStopHook = stopGroups?
            .compactMap { $0["hooks"] as? [[String: Any]] }
            .flatMap { $0 }
            .first(where: { $0["command"] as? String == "'/tmp/AislandHooks'" })

        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/AislandHooks'"))
    ...
    @Test
    func codexHookInstallerInstallReplacesLegacyAislandCommands() throws {
        let existing = """
        {
          "hooks": {
            "PreToolUse": [
              {
                ...
                "hooks": [
                  {
                    "type": "command",
                    "command": "'/Users/test/.aisland/bin/aisland-bridge' --source codex"
                  },
                  {
                    "type": "command",
            ...
            "Stop": [
              {
                "hooks": [
                  ...
                  {
                    "type": "command",
                    "command": "'/tmp/old-debug/AislandHooks'"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true"
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            existingData: existing,
            hookCommand: "'/tmp/new-release/AislandHooks'"
        )
        ...
        #expect(hooks?["PostToolUse"] == nil)
        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/new-release/AislandHooks'"))
        #expect(!stopCommands.contains("'/Users/test/.aisland/bin/aisland-bridge' --source codex"))
        #expect(!stopCommands.contains("'/tmp/old-debug/AislandHooks'"))
    }
    ...
    func codexHookInstallerUninstallRemovesOnlyManagedHooks() throws {
            ...
            "Stop": [
              {
                "hooks": [
                  {
                    "type": "command",
                    "command": "'/tmp/AislandHooks'",
                    "statusMessage": "Managed by Aisland"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true",
        ...
        let mutation = try CodexHookInstaller.uninstallHooksJSON(
            existingData: existing,
            managedCommand: "'/tmp/AislandHooks'"
        )
    ...
    func codexHookPayloadCarriesSessionLocatorIntoJumpTarget() {
        ...
        #expect(jumpTarget.paneTitle == "codex ~/P/aisland")
    }

    @Test
    func codexHookInstallationManagerRoundTripsInstallAndUninstall() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("aisland-tests-\(UUID().uuidString)", isDirectory: true)
        ...
        let managedHooksBinaryURL = rootURL
            ...
            .appendingPathComponent("AislandHooks")
