...
final class TerminalJumpServiceTests: XCTestCase {
    ...
    func testGhosttyJumpScriptFallsBackToWorkingDirectoryAndTitle() {
        ...
        XCTAssertTrue(script.contains("(working directory of aTerminal as text) is \"/Users/wangruobing/Personal/aisland\""))
        XCTAssertTrue(script.contains("(name of aTerminal as text) contains \"codex ~/p/aisland\""))
        XCTAssertTrue(script.contains("if \"\" is \"\" then"))
    }

    func testGhosttyJumpIntegrationMatchesFocusedTerminalForLiveSurfaces() throws {
        guard ProcessInfo.processInfo.environment["AISLAND_RUN_GHOSTTY_JUMP_INTEGRATION"] == "1" else {
            throw XCTSkip("Set AISLAND_RUN_GHOSTTY_JUMP_INTEGRATION=1 to run live Ghostty jump verification.")
        }
