#!/usr/bin/env python3
"""
Aisland hook client — portable Python implementation.

Drop-in replacement for the Swift AislandHooks binary.
...
Usage (configured in Claude Code settings.json or Codex hooks.json):
    echo '{"hook_event_name":"SessionStart",...}' | python3 aisland-hooks.py --source claude
"""
...
def socket_path():
    path = os.environ.get("AISLAND_SOCKET_PATH") or \
           os.environ.get("VIBE_ISLAND_SOCKET_PATH")
    if path:
        return path
    return f"/tmp/aisland-{os.getuid()}.sock"
